...
export default class LineEffect {
  constructor(p0, p1, w, color, durationInSeconds) {
    ...
    if (!Check.num (w,0)) throw new Error(`Width must be a number >0 [${w}]`);
    if (!Check .num (durationInSeconds,0)) throw new Error(`Duration must be a number >0 [${durationInSeconds}]`);
    this.p0 = p0;
    this.p1 = p1;
    this.w = w;
    this.color = color;
  ...
  draw(context, delta) {
    ...
    if (!Check.obj(context)) throw new Error (`LineEffect.draw: bad context parameter${context}`);
    if (!Check.num(delta)) throw (`LineEffect.draw: bad delta parameter delta ${delta}`);    
