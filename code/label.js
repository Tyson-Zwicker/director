...
export default class Label {
  constructor(parent, position, appearance, size, text) {    
    ...
    if (!(Check.obj (appearance , Appearance))) throw new Error('appearance must be an Appearance instance');
    if (!(Check.num(size,0))) throw new Error(`Label.constructor: size must be a number >0 ${size}`);
    this.parent = parent;
